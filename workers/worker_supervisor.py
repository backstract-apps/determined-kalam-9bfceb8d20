"""
Worker Supervisor for coll_a2ea7f17d37e4f23a3ae2df1a29c0d3f
...
COLLECTION_ID = "coll_a2ea7f17d37e4f23a3ae2df1a29c0d3f"
_telemetry_logging_setup_pid = None


def _setup_worker_telemetry_and_logging() -> None:
    """Use the generated app telemetry/logging setup when available."""
    global _telemetry_logging_setup_pid
    current_pid = os.getpid()
    if _telemetry_logging_setup_pid == current_pid:
        return
    ...
    try:
        ...
        _telemetry_logging_setup_pid = current_pid
    except Exception as e:
        logger.warning(f"Worker telemetry/logging setup skipped: {e}")


_setup_worker_telemetry_and_logging()


def _normalize_resource_env_id(resource_id: str) -> str:
    norm = re.sub(r"[^A-Za-z0-9_]", "_", str(resource_id or ""))
    if not norm or norm[0].isdigit():
        norm = f"R_{norm}"
    return norm


def _get_queue_credentials_from_env(queue_resource_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    Resolve queue credentials from generated-app env vars.
    Resource-scoped env vars use normalized ids: QUEUE_<resource_id>_HOST, etc.
    """
    if queue_resource_id:
        env_resource_id = _normalize_resource_env_id(queue_resource_id)
        prefix = f"QUEUE_{env_resource_id}_"
        host = os.getenv(prefix + "HOST")
        if host:
            return {
                "host": host,
                "port": int(os.getenv(prefix + "PORT", "5672")),
                "username": os.getenv(prefix + "USERNAME", "guest"),
                "password": os.getenv(prefix + "PASSWORD", "guest"),
                "vhost": os.getenv(prefix + "VHOST", "/"),
                "ssl": os.getenv(prefix + "SSL", "false").lower() == "true",
            }
        return None
...
def _configure_multiprocessing_start_method() -> None:
    """Use spawn for worker children without changing an already configured runtime."""
    current_method = multiprocessing.get_start_method(allow_none=True)
    if current_method is None:
        multiprocessing.set_start_method("spawn")
        logger.info("Configured multiprocessing start method: spawn")
    elif current_method != "spawn":
        logger.warning(
            "Multiprocessing start method is already configured as "
            f"{current_method!r}; worker child processes may inherit parent sockets"
        )
    ...
    try:
        ...
        try:
            ...
            logger.error(
                ...
                f"worker_id={worker_data.get('worker_id')} "
                f"worker_name={worker_data.get('name')} "
                f"job_id={job_id} "
        ...
        try:
            ...
            logger.info(
                ...
                f"collection_id={COLLECTION_ID} "
                f"worker_id={worker_data['worker_id']} "
                f"worker_name={worker_data.get('name')} "
                f"job_id={job_id}"
            )
...
class WorkerSupervisor:
  ...
  'max_memory_mb': 512,
  'name': 'Test Worker',
  'queue_name': 'test_queue_coll_a2ea7f17d37e4f23a3ae2df1a29c0d3f',
  'queue_resource_id': None,
  'queue_type': 'rabbitmq',
  'queue_use_localhost_defaults': True,
  'queues': [{'queue_name': 'test_queue_coll_a2ea7f17d37e4f23a3ae2df1a29c0d3f',
              'queue_resource_id': None,
              'queue_type': 'rabbitmq',
              'queue_use_localhost_defaults': True}],
  ...
  'worker_id': 'test_worker_coll_a2ea7f17d37e4f23a3ae2df1a29c0d3f'}]
    ...
    async def _get_queue_credentials(self, worker_data: Dict[str, Any], queue_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        env_credentials = (
            _get_queue_credentials_from_env(queue_resource_id)
            if queue_resource_id
            else _get_queue_credentials_from_env()
        )
        if env_credentials:
            logger.info(
                f"[WORKER_SUPERVISOR] Using queue credentials from env for resource_id='{queue_resource_id or 'default'}': "
                f"host={env_credentials['host']}, port={env_credentials['port']}, username={env_credentials['username']}, "
                f"vhost={env_credentials['vhost']}, ssl={env_credentials['ssl']}"
            )
            return env_credentials

        # Platform-provisioned queues must still provide env credentials in deployed apps.
        if q.get("queue_use_localhost_defaults", worker_data.get("queue_use_localhost_defaults")):
            credentials = _get_queue_credentials_from_env()
            if credentials:
                logger.info(
                    "[WORKER_SUPERVISOR] Using default RABBITMQ_* credentials for platform-provisioned queue: "
                    f"host={credentials['host']}, port={credentials['port']}, username={credentials['username']}, "
                    f"vhost={credentials['vhost']}, ssl={credentials['ssl']}"
                )
                return credentials
            raise RuntimeError(
                "Missing queue credentials: expected QUEUE_<resource_id>_* or RABBITMQ_* env vars. "
                f"resource_id={queue_resource_id or 'default'}"
            )
        ...
        if queue_resource_id and _resource_manager:
            try:
                ...
                if mq_credentials:
                    if not mq_credentials.get("host"):
                        raise RuntimeError(f"Missing host in queue credentials for resource_id={queue_resource_id}")
                    credentials = {
                        "host": mq_credentials["host"],
                        ...
                        "ssl": _as_bool(mq_credentials.get("ssl", False)),
                    }
            ...
            credentials = _get_queue_credentials_from_env(queue_resource_id) or _get_queue_credentials_from_env()
            if not credentials:
                raise RuntimeError(
                    "Missing queue credentials: ResourceManager returned no credentials and no "
                    f"QUEUE_* or RABBITMQ_* env vars were found for resource_id={queue_resource_id}"
                )
        else:
            # No resource_id, use default env vars
            credentials = _get_queue_credentials_from_env()
            if not credentials:
                raise RuntimeError("Missing queue credentials: expected RABBITMQ_* env vars")
        return credentials
    ...
    async def _timeout_process(self, process: multiprocessing.Process, job_id: str, timeout: int):
        ...
        if process.is_alive():
            logger.error(
                ...
                f"error=Worker process exceeded timeout of {timeout} seconds"
            )
            process.terminate()
            process.join(timeout=5)
            if process.is_alive():
    ...
    async def _monitor_process(self, job_id: str, process: multiprocessing.Process):
        ...
        try:
            ...
            if job_id in self.running_processes:
                del self.running_processes[job_id]
            if exitcode:
                raise RuntimeError(f"Worker process failed with exit code {exitcode}")
        except Exception as e:
